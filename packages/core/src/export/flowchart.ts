...
import { NodeKind, ProcessGraph, ProcessNode, StateMachineIR, StateNode, StateTransition } from '../model/ir';
...
const FILL: Record<NodeKind | 'start' | 'end', string> = {
  start: '#2E7D32',
  end: '#B00020',
  io: '#1565C0',
  ui: '#00838F',
...
export function businessLabel(text: string): string {
  let t = text;
 ...
 */
export function renderEntryDiagram(
  projectName: string,
  graph: ProcessGraph,
  steps: string[] = [],
  stateSteps?: Record<string, string[]>
): FlowchartImage {
  ...
  if (hasHighLevelSteps(steps)) return renderHighLevelFlow(projectName, steps);
  return renderTechnicalFlow(projectName, graph);
}

/** True when the project is built on the UiPath REFramework (state machine). */
export function isReframework(graph: ProcessGraph): boolean {
  ...
  if (/\bperformer\b|_performer|performer$/.test(name)) return true;
  return false;
}
 ...
 */
export function renderReframeworkStates(): FlowchartImage {
  const W = 760;
  const H = 470;
  const bw = 240;
  const bh = 96;
  ...
  parts.push(
    ...
    `<defs><marker id="arrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L8,3 L0,6 z" fill="#37474F"/></marker></defs>`,
    `<rect x="0" y="0" width="${W}" height="${H}" fill="#FFFFFF"/>`,
    `<text x="${W / 2}" y="34" fill="#263238" font-size="17" font-weight="700" text-anchor="middle">REFramework — State Machine</text>`
  );

  const arrow = (x1: number, y1: number, x2: number, y2: number, label: string, lx: number, ly: number, dashed = false) =>
    `<path d="M${x1},${y1} L${x2},${y2}" fill="none" stroke="#37474F" stroke-width="1.8" ${dashed ? 'stroke-dasharray="5 4"' : ''} marker-end="url(#arrow)"/>` +
  ...
  for (const s of Object.values(states)) {
    parts.push(
      `<rect x="${s.cx - bw / 2}" y="${s.cy - bh / 2}" width="${bw}" height="${bh}" rx="12" ry="12" fill="${s.fill}"/>`,
      `<text x="${s.cx}" y="${s.cy - 6}" fill="#FFFFFF" font-size="16" font-weight="700" text-anchor="middle">${esc(s.title)}</text>`,
      `<text x="${s.cx}" y="${s.cy + 16}" fill="#FFFFFF" font-size="11" text-anchor="middle" opacity="0.9">${esc(s.sub)}</text>`
    );
  }
...
// ----------------------------------------------------------------------------
// Architecture diagram — the robot at the centre, connected to its systems.
// ----------------------------------------------------------------------------

export interface ArchSystem {
  name: string;
  method?: string; // API / UI / DB / Queue
}
 ...
 */
export function renderArchitecture(projectName: string, systems: ArchSystem[]): FlowchartImage {
  ...
  parts.push(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" font-family="Segoe UI, Arial, sans-serif">`,
    `<defs><marker id="arrt" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L8,3 L0,6 z" fill="#37474F"/></marker>` +
      `<marker id="arst" markerWidth="10" markerHeight="10" refX="1" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M8,0 L0,3 L8,6 z" fill="#37474F"/></marker></defs>`,
    `<rect x="0" y="0" width="${W}" height="${H}" fill="#FFFFFF"/>`
  );

  const sysBox = (x: number, cy: number, s: ArchSystem, fill: string) =>
    `<rect x="${x}" y="${cy - boxH / 2}" width="${boxW}" height="${boxH}" rx="9" fill="${fill}"/>` +
  ...
  // Robot (centre-left).
  parts.push(
    `<rect x="${centreX}" y="${robotCy - boxH / 2 - 6}" width="${boxW}" height="${boxH + 12}" rx="12" fill="#0D47A1"/>`,
    `<text x="${robotCx}" y="${robotCy - 4}" fill="#FFFFFF" font-size="15" font-weight="700" text-anchor="middle">UiPath Robot</text>`,
    `<text x="${robotCx}" y="${robotCy + 14}" fill="#FFFFFF" font-size="11" text-anchor="middle" opacity="0.9">${esc(truncate(projectName, 30))}</text>`
  );
  ...
  others.forEach((s, i) => {
    ...
    parts.push(
      `<path d="M${robotCx + boxW / 2},${robotCy} L${rightX},${cy}" fill="none" stroke="#37474F" stroke-width="1.6" marker-end="url(#arrt)" marker-start="url(#arst)"/>`
    );
  });
...
function wrapTspans(text: string, cx: number, y0: number, perLine: number, lineH: number, maxLines: number, fontSize: number, color: string, weight = '600'): string {
 ...
 */
export function renderAgenticFlow(agentName: string, spec?: AgentSpec): FlowchartImage {
  const W = 820;
  const spineX = 250;
  const spineW = 300;
  const rightX = 520;
  ...
  const cyOf = (i: number) => startY + i * (boxH + gap) + boxH / 2;
  const H = cyOf(spineBoxes.length - 1) + boxH / 2 + 30;

  const parts: string[] = [];
  parts.push(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" font-family="Segoe UI, Arial, sans-serif">`,
    `<defs><marker id="aarr" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L8,3 L0,6 z" fill="#37474F"/></marker>` +
      `<marker id="aback" markerWidth="10" markerHeight="10" refX="1" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M8,0 L0,3 L8,6 z" fill="#37474F"/></marker></defs>`,
    `<rect x="0" y="0" width="${W}" height="${H}" fill="#FFFFFF"/>`
  );

  parts.push(
    `<rect x="30" y="${bandY}" width="${W - 60}" height="${bandH}" rx="8" fill="#37474F"/>`,
    `<text x="${W / 2}" y="${bandY + bandH / 2 + 5}" fill="#FFFFFF" font-size="14" font-weight="700" text-anchor="middle">UiPath Orchestrator / Maestro — orchestration, routing &amp; governance</text>`
  );
  ...
  const box = (x: number, cy: number, w: number, h: number, title: string, sub: string, fill: string, dark = false) => {
    const color = dark ? '#212121' : '#FFFFFF';
    return (
      `<rect x="${x}" y="${cy - h / 2}" width="${w}" height="${h}" rx="10" fill="${fill}"/>` +
  ...
  parts.push(
    `<path d="M${spineX + spineW / 2},${guardCy} L${rightX},${guardCy}" fill="none" stroke="#B00020" stroke-width="1.6" marker-end="url(#aarr)"/>`,
    `<rect x="${(spineX + spineW / 2 + rightX) / 2 - 26}" y="${guardCy - 20}" width="52" height="15" rx="3" fill="#FFFFFF"/>`,
    `<text x="${(spineX + spineW / 2 + rightX) / 2}" y="${guardCy - 9}" fill="#B00020" font-size="10" font-weight="700" text-anchor="middle">escalate</text>`
  );
 ...
 */
export function renderAgenticEcosystem(agentName: string, spec?: AgentSpec): FlowchartImage {
  interface Spoke { name: string; sub?: string; fill: string; }
  const spokes: Spoke[] = [];

  const toolNames = (spec?.tools ?? []).map((t) => t.name);
  if (toolNames.length) spokes.push({ name: 'Tools & Integrations', sub: toolNames.slice(0, 3).join(', '), fill: '#00838F' });
  if (spec?.knowledge?.length) spokes.push({ name: 'Context & Knowledge', sub: spec.knowledge.slice(0, 3).map((k) => k.name).join(', '), fill: '#283593' });
  if (spec?.escalations?.length) spokes.push({ name: 'Human-in-the-Loop', sub: spec.escalations.slice(0, 2).map((e) => e.name).join(', '), fill: '#B00020' });
  ...
  parts.push(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" font-family="Segoe UI, Arial, sans-serif">`,
    `<defs><marker id="earr" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L8,3 L0,6 z" fill="#37474F"/></marker>` +
      `<marker id="eback" markerWidth="10" markerHeight="10" refX="1" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M8,0 L0,3 L8,6 z" fill="#37474F"/></marker></defs>`,
    `<rect x="0" y="0" width="${W}" height="${H}" fill="#FFFFFF"/>`
  );

  const sysBox = (x: number, cy: number, name: string, sub: string | undefined, fill: string) =>
    `<rect x="${x}" y="${cy - boxH / 2}" width="${boxW}" height="${boxH}" rx="9" fill="${fill}"/>` +
  ...
  parts.push(
    `<rect x="${centreX}" y="${agentCy - boxH / 2 - 8}" width="${boxW}" height="${boxH + 16}" rx="12" fill="#0D47A1"/>`,
    `<text x="${agentCx}" y="${agentCy - 4}" fill="#FFFFFF" font-size="15" font-weight="700" text-anchor="middle">AI Agent</text>`,
    `<text x="${agentCx}" y="${agentCy + 15}" fill="#FFFFFF" font-size="11" text-anchor="middle" opacity="0.92">${esc(truncate(agentName, 30))}</text>`
  );
...
// ----------------------------------------------------------------------------
// Structured process flow — a branch-aware flowchart built from the real
// activity tree (parentId + branch), replacing the old linear swimlane chain.
// Decisions fork into labelled Yes/No branches that re-merge; loops and
// try/catch render as labelled containers; labels wrap instead of truncating.
// ----------------------------------------------------------------------------
...
interface Laid {
  ...
  w: number;
  h: number;
  cx: number; // entry (top) / exit (bottom) connector x, local coords
}

type FlowItem =
  | { t: 'step'; node: ProcessNode }
  | { t: 'branchy'; head: ProcessNode; shape: 'diamond' | 'rect'; branches: { label?: string; block: FlowItem[] }[] }
  | { t: 'loop'; head: ProcessNode; body: FlowItem[] };
...
export function renderProcessFlow(graph: ProcessGraph): FlowchartImage {
  const byId = new Map(graph.nodes.map((n) => [n.id, n]));
  const kids = new Map<string | undefined, ProcessNode[]>();
  for (const n of graph.nodes) {
    const p = (n.raw?.parentId as string | undefined) ?? undefined;
    if (!kids.has(p)) kids.set(p, []);
    kids.get(p)!.push(n);
  }
  void byId;

  const buildBlock = (parentId: string | undefined): FlowItem[] => {
  ...
  const expand = (nodes: ProcessNode[]): FlowItem[] => {
    const items: FlowItem[] = [];
    for (const n of nodes) {
      if (n.kind === 'sequence') items.push(...buildBlock(n.id));
      else items.push(buildItem(n));
    }
    return items;
  };
  const groupBranches = (children: ProcessNode[]): { label?: string; block: FlowItem[] }[] => {
    const groups: { label?: string; nodes: ProcessNode[] }[] = [];
    let cur: { label?: string; nodes: ProcessNode[] } | null = null;
    for (const c of children) {
      const b = c.raw?.branch as string | undefined;
      if (b || !cur) {
        cur = { label: b, nodes: [] };
        groups.push(cur);
      }
  ...
  const buildItem = (node: ProcessNode): FlowItem => {
    const ch = kids.get(node.id) ?? [];
    if (node.kind === 'if' || node.kind === 'switch') return { t: 'branchy', head: node, shape: 'diamond', branches: groupBranches(ch) };
    if (node.kind === 'loop') return { t: 'loop', head: node, body: buildBlock(node.id) };
    if (node.raw?.activity === 'TryCatch') return { t: 'branchy', head: node, shape: 'rect', branches: groupBranches(ch) };
    if (ch.length) return { t: 'loop', head: node, body: buildBlock(node.id) };
    return { t: 'step', node };
  };
  ...
  { // Cap total rendered nodes so very large processes stay legible.
    let count = 0;
    const trim = (items: FlowItem[]): FlowItem[] => {
      const out: FlowItem[] = [];
      for (const it of items) {
        if (count >= PF.MAX_NODES) { truncated++; continue; }
        count++;
        if (it.t === 'branchy') out.push({ ...it, branches: it.branches.map((b) => ({ ...b, block: trim(b.block) })) });
        else if (it.t === 'loop') out.push({ ...it, body: trim(it.body) });
        else out.push(it);
      }
      return out;
    };
  ...
  const term = (label: string, fill: string, cx: number): Laid => {
    const w = 130, h = 40;
  ...
  let y = pad;
  const parts: string[] = [];
  const place = (l: Laid, dx: number, dy: number) => parts.push(`<g transform="translate(${dx},${dy})">${l.svg}</g>`);
  const vline = (x: number, y1: number, y2: number) => parts.push(`<line x1="${x}" y1="${y1}" x2="${x}" y2="${y2 - 2}" stroke="${PF.ARROW}" stroke-width="1.7" marker-end="url(#pfarr)"/>`);
  ...
  const svg =
    `<svg xmlns="http://www.w3.org/2000/svg" width="${totalW}" height="${totalH}" viewBox="0 0 ${totalW} ${totalH}" font-family="Segoe UI, Arial, sans-serif">` +
    ...
    parts.join('') +
    `</svg>`;
  const resvg = new Resvg(svg, { fitTo: { mode: 'width', value: totalW * 2 } });
  return { png: Buffer.from(resvg.render().asPng()), width: totalW, height: totalH };
}

function layoutBlock(items: FlowItem[]): Laid {
  if (!items.length) return { svg: '', w: PF.NODE_W, h: 0, cx: PF.NODE_W / 2 };
...
function layoutItem(item: FlowItem): Laid {
  if (item.t === 'step') return layoutStep(item.node);
  if (item.t === 'loop') return layoutLoop(item);
  return layoutBranchy(item);
}

function stepColor(node: ProcessNode): string {
  return FILL[node.kind] ?? FILL.other;
}

function nodeBox(node: ProcessNode, w: number, h: number): string {
  ...
  return (
    `<rect x="0" y="0" width="${w}" height="${h}" rx="8" fill="${fill}"/>` +
...
function layoutStep(node: ProcessNode): Laid {
  return { svg: nodeBox(node, PF.NODE_W, PF.NODE_H), w: PF.NODE_W, h: PF.NODE_H, cx: PF.NODE_W / 2 };
}

function layoutLoop(item: FlowItem & { t: 'loop' }): Laid {
...
function layoutBranchy(item: FlowItem & { t: 'branchy' }): Laid {
  ...
  const parts: string[] = [];
  // Head shape.
  if (isDiamond) parts.push(headDiamond(item.head, headW, headH));
  else parts.push(`<g transform="translate(${headX},0)">${nodeBox(item.head, headW, headH)}</g>`);

  // Main branch straight down the spine.
  const mainDx = spineX - main.L.cx;
  parts.push(`<line x1="${spineX}" y1="${headH}" x2="${spineX}" y2="${branchTop - 2}" stroke="${PF.ARROW}" stroke-width="1.7" marker-end="url(#pfarr)"/>`);
  if (main.label) parts.push(branchLabel(spineX, headH + 12, main.label));
  parts.push(`<g transform="translate(${mainDx},${branchTop})">${main.L.svg}</g>`);
  const bottoms: { x: number; y: number }[] = [{ x: spineX, y: branchTop + main.L.h }];
  ...
  for (const o of others) {
    ...
    if (o.label) parts.push(branchLabel((spineX + oEntry) / 2, branchTop - PF.LABEL_H - 4, o.label));
  ...
  for (const b of bottoms) {
    if (b.x === spineX) parts.push(`<line x1="${spineX}" y1="${b.y}" x2="${spineX}" y2="${mergeY}" stroke="${PF.ARROW}" stroke-width="1.5"/>`);
    else parts.push(`<path d="M${b.x},${b.y} L${b.x},${mergeY} L${spineX},${mergeY}" fill="none" stroke="${PF.ARROW}" stroke-width="1.5"/>`);
  }

  return { svg: parts.join(''), w: cursorRight, h: mergeY, cx: spineX };
}

function headDiamond(node: ProcessNode, w: number, h: number): string {
  ...
  return (
    `<polygon points="${cx},0 ${w},${cy} ${cx},${h} 0,${cy}" fill="${fill}" stroke="#00000022"/>` +
...
function branchLabel(x: number, y: number, label: string): string {
  const w = label.length * 6.4 + 10;
  return (
    `<rect x="${x - w / 2}" y="${y - 11}" width="${w}" height="15" rx="3" fill="#FFFFFF" stroke="#CFD8DC" stroke-width="0.75"/>` +
...
export function hasHighLevelSteps(steps: string[] | undefined): boolean {
 ...
 */
export function renderHighLevelFlow(title: string, stepsIn: string[]): FlowchartImage {
  ...
  const heights: number[] = [termH, ...steps.map(() => stepH), termH];
  let H = padTop + padBot;
  heights.forEach((h, i) => { H += h; if (i < rows - 1) H += gap; });

  const centers: number[] = [];
  let y = padTop;
  for (let i = 0; i < rows; i++) { centers.push(y + heights[i] / 2); y += heights[i] + gap; }
  ...
  parts.push(
    ...
    `<rect x="0" y="0" width="${W}" height="${H}" fill="#FFFFFF"/>`,
    `<text x="${cx}" y="30" fill="#263238" font-size="15" font-weight="700" text-anchor="middle">High-level process flow${title ? ` — ${esc(truncate(title, 40))}` : ''}</text>`
  );
  ...
  parts.push(
    ...
    `<text x="${cx}" y="${centers[0] + 5}" fill="#fff" font-size="15" font-weight="700" text-anchor="middle">Start</text>`
  );
  ...
  steps.forEach((s, i) => {
    ...
    // Number badge.
    parts.push(
      `<circle cx="${x + 26}" cy="${cy}" r="15" fill="#FFFFFF" opacity="0.9"/>`,
      `<text x="${x + 26}" y="${cy + 5}" fill="${fill}" font-size="14" font-weight="700" text-anchor="middle">${i + 1}</text>`
    );
  ...
  parts.push(
    ...
    `<text x="${cx}" y="${eCy + 5}" fill="#fff" font-size="15" font-weight="700" text-anchor="middle">End</text>`
  );
...
const SWIM_STATES: { key: 'init' | 'get' | 'process' | 'end'; name: string; color: string }[] = [
...
function bucketSteps(steps: string[]): Record<'init' | 'get' | 'process' | 'end', string[]> {
  ...
  let seenGet = false;
  clean.forEach((s) => {
    if (RX_END.test(s) && seenGet) b.end.push(s); // teardown only counts after processing began
    else if (!seenGet && RX_GET.test(s)) { b.get.push(s); seenGet = true; }
    else if (!seenGet) b.init.push(s); // everything before the first fetch is setup
    else if (RX_END.test(s)) b.end.push(s);
  ...
  if (!seenGet && b.process.length === 0) {
    ...
    clean.forEach((s, i) => {
      ...
      else if (RX_INIT.test(s) || i === 0) b.init.push(s);
      else b.process.push(s);
    });
  }
  return b;
...
function isDecisionStep(s: string): boolean {
...
// A node on one REFramework lane's vertical spine.
type LaneNode = {
  kind: 'terminal' | 'process' | 'decision' | 'connector';
  label: string;
  sideTo?: 'A' | 'B'; // decision's branch exit routes to this off-page connector
  sideLabel?: string; // label on the branch exit (YES/NO/Yes/No)
  downLabel?: string; // label on the continue-down exit
  exception?: string; // exception tag drawn by the branch (SE#1/BE#1…)
  retry?: boolean; // dashed "Retry" loop back to the previous process node
  entry?: boolean; // an incoming off-page connector at the top of a lane
};

/** Ensure each REFramework lane has at least canonical steps when the LLM gave none. */
function bucketFallback(buckets: Record<'init' | 'get' | 'process' | 'end', string[]>): Record<'init' | 'get' | 'process' | 'end', string[]> {
  return {
...
function buildLaneNodes(bucketsIn: Record<'init' | 'get' | 'process' | 'end', string[]>): Record<'init' | 'get' | 'process' | 'end', LaneNode[]> {
  const b = bucketFallback(bucketsIn);
  const proc = (label: string, extra: Partial<LaneNode> = {}): LaneNode => ({ kind: 'process', label, ...extra });
  const dec = (label: string, sideLabel: string, sideTo: 'A' | 'B', downLabel: string, exception?: string, retry = false): LaneNode =>
    ({ kind: 'decision', label, sideLabel, sideTo, downLabel, exception, retry });
  ...
  const init: LaneNode[] = [{ kind: 'terminal', label: 'Start' }];
  ...
  const get: LaneNode[] = [{ kind: 'connector', label: 'B', entry: true }];
  ...
  const end: LaneNode[] = [{ kind: 'connector', label: 'A', entry: true }];
...
// ----------------------------------------------------------------------------
// High-level "technical flow" — a readable, categorized summary of what the
// entry workflow DOES (UI actions, data I/O, decisions, loops, sub-processes),
// instead of a per-activity dump of every assign / variable / view-state type.
// ----------------------------------------------------------------------------

type TechCat = 'ui' | 'io' | 'decision' | 'loop' | 'invoke' | 'log' | 'trycatch' | 'assign' | 'step';
const TECH_STYLE: Record<TechCat, { tag: string; color: string; shape: 'box' | 'decision' }> = {
...
function techCategory(n: ProcessNode): TechCat {
  const name = n.displayName;
  const k = n.kind;
  if (k === 'if' || k === 'switch') return 'decision';
  if (k === 'loop' || /\bfor each\b|\bwhile\b|\bdo while\b|\brepeat\b|\bretry scope\b/i.test(name)) return 'loop';
  if (/try ?catch/i.test(name)) return 'trycatch';
  if (k === 'invoke' || /\binvoke\b/i.test(name)) return 'invoke';
  ...
  if (k === 'io' || /excel|workbook|read range|write range|write cell|read cell|data ?table|insert column|use excel|read csv|write csv|read text|write text|append line|read pdf|mail|outlook|send email|get email|word document|build data|filter data|for each excel/i.test(name))
    return 'io';
  if (k === 'log' || /message box|log message|log error|write line|report status|input dialog/i.test(name)) return 'log';
  if (k === 'assign' || /\bassign\b|multiple assign|^set /i.test(name)) return 'assign';
  return 'step';
}

/** A high-level step in the technical flow. */
interface TechStep {
  cat: TechCat;
  label: string;
}
...
function cleanTechLabel(cat: TechCat, nameIn: string): string {
  let s = nameIn.trim();
  ...
  if (cat === 'invoke') {
    ...
    if (m) return `Invoke ${m[1].replace(/[_-]+/g, ' ')}`;
 ...
 */
function techSteps(graph: ProcessGraph, max = 18): { steps: TechStep[]; dropped: number; hasTryCatch: boolean } {
  const entry = (graph.entryPoints[0] || '').split(/[\\/]/).pop()?.toLowerCase();
  const inEntry = (n: ProcessNode) => {
  ...
  if (pool.filter((n) => techCategory(n) !== 'assign' && !TECH_DROP.has(n.displayName)).length < 3) pool = graph.nodes;

  const hasTryCatch = pool.some((n) => /try ?catch/i.test(n.displayName));
  const raw: TechStep[] = [];
  for (const n of pool) {
    if (TECH_DROP.has(n.displayName)) continue;
    const cat = techCategory(n);
    if (cat === 'trycatch') continue; // shown as a single band, not a step
    // Skip a bare "Sequence"/phase container with no descriptive name.
    if (n.kind === 'sequence' && /^(sequence|do|body)$/i.test(n.displayName)) continue;
    raw.push({ cat, label: cleanTechLabel(cat, n.displayName) });
  }
  // Collapse consecutive prepare-data assignments into one box.
  const collapsed: TechStep[] = [];
  for (const s of raw) {
  ...
  if (collapsed.length > max) {
    const priority = (c: TechCat) => (c === 'assign' || c === 'log' || c === 'step' ? 0 : 1);
    ...
    for (const s of collapsed) {
      if (toDrop > 0 && priority(s.cat) === 0) { toDrop--; dropped++; continue; }
      keep.push(s);
    }
 ...
 */
export function renderTechnicalFlow(title: string, graph: ProcessGraph): FlowchartImage {
 ...
 */
function isExceptionTransition(name: string, _condition?: string): boolean {
...
export function stateStepsFromFlows(
  flows: { state: string; steps: string[] }[] | undefined
): Record<string, string[]> | undefined {
  if (!flows?.length) return undefined;
  const out: Record<string, string[]> = {};
  for (const f of flows) if (f.state && f.steps?.length) out[normState(f.state)] = f.steps;
  return Object.keys(out).length ? out : undefined;
}

/** Canonicalise common REFramework activity names so near-duplicates collapse. */
function canonicalStep(s: string): string {
  if (/get.*asset|asset.*orchestrator|orchestrator asset/i.test(s)) return 'Retrieve Orchestrator assets';
  if (/parse client|client (id|name|country|information)/i.test(s)) return 'Parse client details';
  if (/(local )?settings and constants|read config|get.*config/i.test(s)) return 'Read config settings';
  if (/read range/i.test(s)) return 'Read config settings';
  if (/get.*credential/i.test(s)) return 'Get credential from Orchestrator';
  return s;
}
 ...
 */
export function deriveStateSteps(graph: ProcessGraph): Record<string, string[]> {
  const sm = graph.stateMachine;
  if (!sm?.states?.length) return {};
  const byFile = new Map<string, ProcessNode[]>();
  for (const n of graph.nodes) {
  ...
  for (const st of sm.states) {
    ...
    for (const wf of st.steps) {
      for (const n of byFile.get(wf.toLowerCase()) ?? []) {
        ...
        if (!dn || STEP_DROP.test(dn) || !(STEP_KEEP.test(dn) || n.kind === 'invoke')) continue;
    ...
    if (steps.length) out[normState(st.name)] = steps;
  }
  return out;
}

/** Short unique code for each state, used on off-page connectors (e.g. GTD, PT, EP). */
function stateCodes(states: StateNode[]): Record<string, string> {
  const out: Record<string, string> = {};
  ...
  for (const s of states) {
    ...
    let i = 2;
    while (used.has(code)) code = (code.slice(0, 3) + i++).toUpperCase();
    used.add(code);
    out[s.id] = code;
  }
  return out;
 ...
 */
function orderStates(sm: StateMachineIR): StateNode[] {
  const byId: Record<string, StateNode> = {};
  for (const s of sm.states) byId[s.id] = s;
  const chain: StateNode[] = [];
  const seen = new Set<string>();
  let cur: StateNode | undefined = byId[sm.initial] ?? sm.states[0];
  while (cur && !seen.has(cur.id)) {
    const node: StateNode = cur;
    chain.push(node);
    seen.add(node.id);
    const fwd = node.transitions.find(
      (t) => !isExceptionTransition(t.name, t.condition) && !!byId[t.to] && !seen.has(t.to)
    );
...
function isBusinessExc(t: StateTransition): boolean {
  return /business ?exception|businessrule/i.test(`${t.name} ${t.condition ?? ''}`);
}
function isSystemExc(t: StateTransition): boolean {
...
function shortBranch(t: StateTransition): string {
  if (isBusinessExc(t)) return 'Bus Exc';
  if (isSystemExc(t)) return 'Sys Exc';
  if (/no ?data/i.test(`${t.name} ${t.condition ?? ''}`)) return 'No Data';
  return 'NO';
}
 ...
 */
function decisionQuestion(s: StateNode): string {
  ...
  if (/transactionitem|new transaction|no data|dequeue|queue ?item|next (item|transaction)/.test(g)) return 'New transaction|available?';
  if (isInit && /(system)?exception|failed|error/.test(g)) return 'Initialised|successfully?';
  if (/businessexception|systemexception|exception|failed|error/.test(g)) return 'Processed|successfully?';
...
function diamondLabel(cx: number, cy: number, label: string, maxChars: number): string {
  const lines = label.split('|');
  if (lines.length === 1) return wrapTspans(label, cx, cy - 2, maxChars, 12, 2, 10.5, '#12212E', '700');
  return (
    `<text x="${cx}" y="${cy - 4}" fill="#12212E" font-size="10.5" font-weight="700" text-anchor="middle">${esc(lines[0])}</text>` +
 ...
 */
function stateMachineBlock(title: string, sm: StateMachineIR, w: number, apps?: Record<string, string[]>, stateSteps?: Record<string, string[]>): { svg: string; height: number } {
  ...
  let seN = 0;
  let beN = 0;
  const excTag = (t: StateTransition): string | undefined =>
    isBusinessExc(t) ? `BE#${++beN}` : isSystemExc(t) ? `SE#${++seN}` : undefined;
  ...
  const drawConn = (px: number, py: number, code: string, exc: boolean) => {
  ...
  const arrow = (x1: number, y1: number, x2: number, y2: number, stroke = INK, width = 1.5, dashed = false) =>
    parts.push(`<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${stroke}" stroke-width="${width}" ${dashed ? 'stroke-dasharray="5 4"' : ''} marker-end="url(#pfarrow)"/>`);
  ...
  ordered.forEach((s, i) => {
    ...
    const primary =
      happy.find((t) => orderIdx[t.to] === i + 1) ??
      happy[0] ??
      exits[0];
    ...
    if (exits.length) {
      ...
      const busX = cx + dW / 2;
      const STAG = 60;
      if (others.length > 1) parts.push(`<line x1="${busX}" y1="${dcy}" x2="${busX}" y2="${dcy + (others.length - 1) * STAG}" stroke="${INK}" stroke-width="1.3"/>`);
      let lowestSide = dcy;
      others.forEach((t, k) => {
        const ry = dcy + k * STAG;
        ...
        if (tag) parts.push(`<text x="${connX}" y="${ry + 26}" fill="#B00020" font-size="9.5" font-weight="700" text-anchor="middle">${esc(tag)}</text>`);
 ...
 */
function stateSwimlaneBlock(title: string, steps: string[], w: number): { svg: string; height: number } {
  ...
  const nodeH = (n: LaneNode) => (n.kind === 'terminal' ? 34 : n.kind === 'connector' ? 34 : n.kind === 'decision' ? 56 : 48);
  const parts: string[] = [];
  ...
  const KEYS: ('init' | 'get' | 'process' | 'end')[] = ['init', 'get', 'process', 'end'];
  type Laid = { n: LaneNode; cy: number; h: number };
  const geom: { key: string; cx: number; laneX: number; connX: number; bw: number; dhw: number; laid: Laid[]; entryIdx: number }[] = [];
  let maxBottom = bodyTop;
  ...
  KEYS.forEach((key, i) => {
    ...
    (geom[geom.length - 1] as any).color = color;
  });
  ...
  const drawConnector = (px: number, py: number, letter: string) => {
  ...
  const routeToNext = (from: typeof geom[number], to: typeof geom[number]) => {
    ...
    const midY = tgt.cy;
    parts.push(
      `<path d="M${sx},${sy} V${sy + 12} H${boundary - 0} V${midY} H${tgtLeft - 2}" fill="none" stroke="#546E7A" stroke-width="1.5" marker-end="url(#pfarrow)"/>`
    );
  };

  // ---- Draw each lane ----
  geom.forEach((g) => {
    const color = (g as any).color as string;
    ...
    for (let k = 0; k < g.laid.length - 1; k++) {
      ...
      if (label) parts.push(`<text x="${g.cx + 7}" y="${a.cy + a.h / 2 + 13}" fill="#37474F" font-size="9.5" font-weight="600">${esc(label)}</text>`);
    }
    ...
    g.laid.forEach((it, idx) => {
      ...
      if (n.kind === 'terminal') {
        ...
        if (n.sideTo) {
          ...
          if (n.sideLabel) parts.push(`<text x="${g.cx + hw + 8}" y="${cy - 5}" fill="#37474F" font-size="9.5" font-weight="700">${esc(n.sideLabel)}</text>`);
          drawConnector(cxTo, cy, n.sideTo);
          if (n.exception) parts.push(`<text x="${cxTo}" y="${cy + 28}" fill="#B00020" font-size="10" font-weight="700" text-anchor="middle">${esc(n.exception)}</text>`);
        }
        ...
        if (n.retry && idx > 0) {
          ...
          const lx = g.cx - hw - 8;
          parts.push(
            `<path d="M${g.cx - hw},${cy} H${lx} V${prev.cy} H${g.cx - g.bw / 2 - 2}" fill="none" stroke="#90A4AE" stroke-width="1.3" stroke-dasharray="4 3" marker-end="url(#pfarrow)"/>`
          );
 ...
 */
function linearFlowBlock(title: string, stepsIn: string[], w: number): { svg: string; height: number } {
  const steps = stepsIn.map((s) => s.trim()).filter(Boolean).slice(0, 14);
  const cx = w / 2;
  ...
  steps.forEach((s, i) => {
    ...
    parts.push(
      `<circle cx="${x + 24}" cy="${cy}" r="14" fill="#FFFFFF" opacity="0.9"/>`,
      `<text x="${x + 24}" y="${cy + 5}" fill="${fill}" font-size="13" font-weight="700" text-anchor="middle">${i + 1}</text>`
    );
...
function rasterizeLinear(title: string, steps: string[]): FlowchartImage {
...
export function renderPartitionedFlows(
  ...
  if (usable.length <= 1) {
    const f = usable[0];
    if (!f) return renderPartitionedFlow('', []);
    const b = blockFor(f);
    const h = b.height + 20;
