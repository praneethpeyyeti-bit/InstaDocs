 ...
 */
export function folderTree(root: string, maxDepth = 5, maxLines = 200): string {
  ...
  const walk = (dir: string, prefix: string, depth: number): void => {
    ...
    // Show every real file — the folder structure must be complete, not sampled.
    const files = entries
      .filter((e) => e.isFile() && !e.name.startsWith('.'))
      .sort((a, b) => a.name.localeCompare(b.name));
    const items = [
      ...dirs.map((d) => ({ name: d.name, dir: true })),
      ...files.map((f) => ({ name: f.name, dir: false })),
    ];
    items.forEach((it, i) => {
