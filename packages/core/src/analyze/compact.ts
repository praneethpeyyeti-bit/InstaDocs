...
export function compactGraph(graph: ProcessGraph, maxNodes = 180): string {
  ...
  if (graph.stateMachine?.states?.length) {
    ...
    for (const n of graph.nodes) {
      ...
      if (dn && !/^(sequence|body|do|then|else|assign|log message|target|comment)$/i.test(dn)) byFile.get(f)!.push(dn);
    }
    ...
    for (const st of graph.stateMachine.states) {
      ...
      const appsFor = new Set<string>();
      const acts: string[] = [];
      for (const wf of wfs) {
        for (const a of graph.workflowApps?.[wf.toLowerCase()] ?? []) appsFor.add(a);
        for (const dn of byFile.get(wf.toLowerCase()) ?? []) if (acts.length < 10 && /^(get|read|retrieve|extract|parse|calculate|build|validate|update|submit|create|send|download|upload|log ?in|login|open|go to|navigate|close|classify|categor|compose|write|lookup|reply)\b/i.test(dn)) acts.push(dn);
      }
