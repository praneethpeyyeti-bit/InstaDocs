 ...
 */
export async function enrichAdd(
  graph: ProcessGraph,
  options: AddEnrichOptions = {}
): Promise<{ model: AddModel; usedLlm: boolean }> {
  ...
  const userPrompt = [
    ...
    `Platform: ${PLATFORM_LABELS[graph.platform]} (Agentic)`,
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    try {
      ...
      const model = AddModelSchema.parse(extractJson(raw));
  ...
  throw new Error(
    `LLM Gateway enrichment failed after ${maxRetries + 1} attempt(s): ${(lastErr as Error)?.message ?? lastErr}`
  );
}

/** Ensure the control tables always have at least the baseline structure. */
function backfill(model: AddModel, graph: ProcessGraph): void {
...
function extractJson(raw: string): unknown {
  ...
  try {
    ...
    if (s >= 0 && e > s) return stripNulls(JSON.parse(t.slice(s, e + 1)));
