 ...
 * mechanical document.
 */
async function enrichSdd(graph, options = {}) {
 ...
 */
async function enrichSddSolution(solutionName, projects, merged, options = {}) {
    const { gateway, maxRetries = 2, onProgress } = options;
    if (!gateway) {
        throw new Error('InstaDocs requires the UiPath LLM Gateway (no deterministic mode). Run `uip login` or set INSTADOCS_GATEWAY_URL/TOKEN.');
    }
    const evidence = projects
        .map((p) => `=== PROJECT: ${p.name} ===\n${(0, compact_1.compactGraph)(p.graph)}`)
        .join('\n\n');
    const userPrompt = [
        `Solution: ${solutionName}`,
        `Platform: ${ir_1.PLATFORM_LABELS[merged.platform]}`,
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
        try {
            ...
            model.platformLabel ||= ir_1.PLATFORM_LABELS[merged.platform];
            if (!model.dependencies.length)
    ...
    throw new Error(`LLM Gateway solution enrichment failed after ${maxRetries + 1} attempt(s): ${lastErr?.message ?? lastErr}`);
}
/** Merge several project graphs into one graph for solution-level fields. */
function mergeGraphs(name, graphs) {
    const merged = (0, ir_1.emptyGraph)('uipath', name);
    ...
    for (const g of graphs) {
        ...
        for (const e of g.entryPoints)
            merged.entryPoints.push(e);
        ...
        for (const q of g.queueItems ?? [])
            (merged.queueItems ??= []).push(q);
    }
    return merged;
}
/**
...
const FRAMEWORK_WF = /InitAllSettings|InitAllApplications|CloseAllApplications|KillAllProcesses|GetTransactionData|SetTransactionStatus|RetryCurrentTransaction|TakeScreenshot|SendExceptionEmail|^Process\b|^Main\b|CloseAllApplications/i;
/**
 ...
 */
function buildQueueItemJson(graph) {
    ...
    for (const it of items) {
        ...
        for (const f of it.fields)
            if (!fields.has(f.name))
                fields.set(f.name, f.type || 'String');
    }
    const specific = {};
    for (const [k, t] of fields)
        specific[k] = jsonExample(t);
...
function jsonExample(type) {
    ...
    if (/int|long|double|decimal|number|float/.test(t))
        return 0;
    if (/bool/.test(t))
        return false;
    if (/date/.test(t))
...
function allDependencies(graph) {
    if (graph.projectContext?.dependencies?.length)
        return graph.projectContext.dependencies;
 ...
 */
function backfillDependencyVersions(model, graph) {
    const real = graph.dependencies ?? [];
    if (!real.length)
        return;
    const find = (name) => real.find((d) => d.package.toLowerCase() === name.toLowerCase()) ??
 ...
 */
function deterministicSdd(graph) {
    const ctx = graph.projectContext;
    const platformLabel = ir_1.PLATFORM_LABELS[graph.platform];
    const deps = deriveDependencies(graph);
    const apps = (0, projectContext_1.dependenciesToApplications)(allDependencies(graph));
    const modules = (ctx?.keyWorkflows ?? []).map((w) => ({
        name: w.workflow,
        parent: '',
        arguments: '',
    ...
    if (!modules.length) {
        for (const inv of graph.invocations)
            modules.push({ name: inv.target, parent: '', arguments: '', reusable: '', folderPath: '', description: 'Invoked workflow.' });
    }
    const exceptions = [];
