...
function sddToMarkdown(m) {
    ...
    h('Architecture');
    p(m.architecture);
    if (m.systemsPrereq.length) {
        h('Systems & Prerequisites');
        table(['System', 'Requisite'], m.systemsPrereq.map((x) => [x.system, x.requisite]));
    }
    if (m.robotInfo.length) {
        h('Robot & Process Information');
        table(['Item', 'Detail'], m.robotInfo.map((x) => [x.item, x.desc]));
    }
    if (m.processes.length) {
        h('Processes');
        table(['Process', 'Folder', 'Description'], m.processes.map((x) => [x.name, x.folderPath, x.description]));
    }
    if (m.queues.length) {
        h('Queues');
        table(['Queue', 'Folder', 'Details'], m.queues.map((x) => [x.name, x.folderPath, x.details]));
    }
    if (m.namingConventions.length) {
        h('Naming Conventions');
        bullets(m.namingConventions);
    }
    if (m.modules.length) {
        h('Technical Process Design — Modules');
        table(['Module', 'Parent', 'Args', 'Reusable', 'Description'], m.modules.map((x) => [x.name, x.parent, x.arguments, x.reusable, x.description]));
    }
    p('\n_High-level process flow diagram is embedded in the Word SDD._');
    if (m.exceptions.length) {
        h('Exceptions');
        table(['Code', 'Detail', 'Type', 'Bot Action'], m.exceptions.map((x) => [x.code, x.detail, x.type, x.botAction]));
    }
    if (m.dependencies.length) {
        h('Dependencies');
        bullets(m.dependencies.map((d) => `**${d.name}**${d.version ? ` (${d.version})` : ''} — ${d.purpose || ''}`));
    }
    if (m.futureImprovements.length) {
        h('Future Improvements');
        bullets(m.futureImprovements);
    }
    p(m.dataSecurity ? `\n## Data Security\n${m.dataSecurity}` : '');
...
function testCasesToMarkdown(m) {
    const L = [`# Test Cases — ${m.projectName}`, ''];
...
//# sourceMappingURL=sddMarkdown.js.map