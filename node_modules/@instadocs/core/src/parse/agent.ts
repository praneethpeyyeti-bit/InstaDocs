...
import { findAgentJson } from '../detect/docType';
 ...
 */
export function parseAgent(workingDir: string): AgentSpec | undefined {
...
function parseAgentJson(file: string, workingDir: string): AgentSpec {
  const obj = JSON.parse(readText(file));
  ...
  const spec = emptyAgentSpec(name, rel, 'lowCode');
...
function extractPrompts(obj: any): { system?: string; user?: string } {
  ...
  for (const m of messages) {
    ...
    if (!content) continue;
    if (/system/i.test(role)) system ||= content;
    else if (/user|human/i.test(role)) user ||= content;
  }
  return { system: system || undefined, user: user || undefined };
}

function extractModel(obj: any): AgentModel | undefined {
  ...
  if (!name && !provider && !temperature && !maxTokens) return undefined;
  const settings: Record<string, string> = {};
  for (const k of ['topP', 'top_p', 'frequencyPenalty', 'presencePenalty', 'seed']) {
    const v = pick(s, k);
    if (v !== undefined) settings[k] = numStr(v) || str(v);
  }
  return {
    name: name || undefined,
...
function inferProvider(model?: string): string | undefined {
  ...
  if (/gpt|o1|o3|davinci|text-embedding/.test(m)) return /azure/.test(m) ? 'Azure OpenAI' : 'OpenAI';
  if (/claude|anthropic/.test(m)) return 'Anthropic';
  if (/gemini|palm/.test(m)) return 'Google';
  if (/llama|mistral|mixtral/.test(m)) return 'Open source';
  return undefined;
}

function extractTools(obj: any): AgentTool[] {
...
function extractArgs(obj: any, dir: 'in' | 'out'): AgentArgument[] {
  ...
  for (const a of arr(list)) {
    if (typeof a === 'string') { args.push({ name: a }); continue; }
...
function extractEscalations(obj: any): AgentEscalation[] {
  const raw = [...arr(pick(obj, 'escalations', 'escalation')), ...arr(pick(pick(obj, 'features', 'humanInTheLoop', 'hitl'), 'escalations'))];
  const out: AgentEscalation[] = [];
  for (const e of raw) {
    if (typeof e === 'string') { out.push({ name: e }); continue; }
...
function extractKnowledge(obj: any): AgentKnowledge[] {
  const raw = [
  ...
  for (const k of raw) {
    if (typeof k === 'string') { out.push({ name: k }); continue; }
...
function extractGuardrails(obj: any): string[] {
  ...
  for (const item of arr(g)) {
    if (typeof item === 'string') out.push(item);
      ...
      if (name || desc) out.push([name, desc].filter(Boolean).join(': '));
    }
  }
...
function parseCodedAgent(workingDir: string): AgentSpec | undefined {
  const py = walkFiles(workingDir, { extensions: ['.py'], maxFiles: 400 });
  if (!py.length) return undefined;
  ...
  if (!AGENT_PY_MARKERS.test(blob) && !AGENT_PY_MARKERS.test(config)) return undefined;

  const name = readCodedName(workingDir);
  const spec = emptyAgentSpec(name, path.relative(workingDir, py[0]) || 'agent', 'coded');
  ...
  if (model) spec.model = { name: model, provider: inferProvider(model) };

  // Tools: @tool decorators + tool names in a tools=[...] list.
  const tools = new Set<string>();
  for (const m of blob.matchAll(/@tool[^\n]*\n\s*def\s+(\w+)/g)) tools.add(m[1]);
  for (const m of blob.matchAll(/def\s+(\w+)\s*\([^)]*\)\s*->/g)) if (/tool|action/i.test(m[1])) tools.add(m[1]);
...
function detectRuntime(blob: string): string | undefined {
  if (/langgraph/i.test(blob)) return 'langgraph';
  if (/llama[_-]?index/i.test(blob)) return 'llamaindex';
  if (/openai[_-]?agents/i.test(blob)) return 'openai-agents';
  if (/uipath[_-]?langchain|from uipath/i.test(blob)) return 'uipath';
  return undefined;
}

function readCodedName(workingDir: string): string {
...
function readCodedEntryPoints(workingDir: string): string[] {
  const uipathJson = path.join(workingDir, 'uipath.json');
  if (!exists(uipathJson)) return [];
  try {
    const obj = JSON.parse(readText(uipathJson));
...
function safe(file: string): string {
  try {
