...
import { NodeKind, ProcessGraph, ProcessNode } from '../model/ir';
...
interface Item {
  kind: NodeKind | 'start' | 'end';
  title: string;
  condition?: string;
}

export function renderFlowchart(graph: ProcessGraph): FlowchartImage {
...
function buildItems(graph: ProcessGraph): Item[] {
  const meaningful = graph.nodes.filter((n) =>
    ['io', 'ui', 'invoke', 'if', 'switch', 'loop', 'throw', 'assign', 'log'].includes(n.kind)
  );
  const chosen = (meaningful.length ? meaningful : graph.nodes).slice(0, MAX_NODES);
  const items: Item[] = [{ kind: 'start', title: 'Start' }];
  for (const n of chosen) items.push(toItem(n));
...
function toItem(n: ProcessNode): Item {
...
export function laneOf(n: ProcessNode): string {
  const name = n.displayName;
  if (/excel|workbook|spreadsheet/i.test(name)) return 'MS Excel';
  if (/mail|email|outlook|smtp|notif/i.test(name)) return 'Email';
  if (/browser|web|http|api|url|navigate/i.test(name)) return 'Web / API';
  if (/sap/i.test(name)) return 'SAP';
  if (/database|sql|query/i.test(name)) return 'Database';
  if (n.kind === 'ui') return 'Business Application';
  if (n.kind === 'invoke') return 'Sub-process';
  return 'RPA Bot';
}

function itemHeight(kind: Item['kind']): number {
  if (kind === 'start' || kind === 'end') return TERM_H;
  if (kind === 'if' || kind === 'switch') return DEC_H;
  return PROC_H;
}

function svgHeight(items: Item[]): number {
  let h = PAD_TOP;
  items.forEach((it, i) => {
...
function buildSvg(items: Item[]): string {
  const height = svgHeight(items);
  const cx = W / 2;
  const parts: string[] = [];

  parts.push(
    `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${height}" viewBox="0 0 ${W} ${height}" font-family="Segoe UI, Arial, sans-serif">`,
    `<defs><marker id="arrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth">` +
      `<path d="M0,0 L8,3 L0,6 z" fill="#37474F"/></marker></defs>`,
  ...
  for (let i = 0; i < items.length - 1; i++) {
    ...
    parts.push(
      `<line x1="${cx}" y1="${y1}" x2="${cx}" y2="${y2 - 2}" stroke="#37474F" stroke-width="1.6" marker-end="url(#arrow)"/>`
    );
  }
...
function shape(it: Item, cx: number, cy: number): string {
...
export function renderSwimlane(graph: ProcessGraph): FlowchartImage {
  ...
  const lanes: string[] = [];
  for (const r of rows) if (!lanes.includes(r.lane)) lanes.push(r.lane);

  const W = SW_PADX * 2 + lanes.length * LANE_W;
  const H = SW_PADTOP + rows.length * ROW_H + SW_PADBOT;
  const laneColor = (lane: string) => LANE_COLORS[lanes.indexOf(lane) % LANE_COLORS.length];
  const laneCx = (lane: string) => SW_PADX + lanes.indexOf(lane) * LANE_W + LANE_W / 2;
  const rowCy = (i: number) => SW_PADTOP + i * ROW_H + ROW_H / 2;
  ...
  lanes.forEach((lane, li) => {
    ...
    parts.push(
      `<rect x="${x + 3}" y="8" width="${LANE_W - 6}" height="${HEADER_H - 10}" rx="6" fill="${laneColor(lane)}"/>`,
      `<text x="${x + LANE_W / 2}" y="${8 + (HEADER_H - 10) / 2 + 5}" fill="#FFFFFF" font-size="14" font-weight="700" text-anchor="middle">${esc(lane)}</text>`,
      `<line x1="${x}" y1="${HEADER_H + 4}" x2="${x}" y2="${H - 6}" stroke="#ECEFF1" stroke-width="1"/>`
    );
  });
...
function halfH(kind: SwimRow['kind']): number {
  if (kind === 'start' || kind === 'end') return 20;
  if (kind === 'if' || kind === 'switch') return 29;
  return 23;
}

function swimShape(r: SwimRow, cx: number, cy: number, color: string): string {
  const bw = LANE_W - 44;
