...
import { NodeKind, ProcessGraph, ProcessNode, StateMachineIR, StateNode } from '../model/ir';
 ...
 */
export function renderEntryDiagram(projectName: string, graph: ProcessGraph, steps: string[] = []): FlowchartImage {
 ...
 */
function stateMachineBlock(title: string, sm: StateMachineIR, w: number): { svg: string; height: number } {
  ...
  const drawConn = (px: number, py: number, code: string, color: string) => {
  ...
  ordered.forEach((s, i) => {
    ...
    if (s.transitions.length) {
      ...
      s.transitions.forEach((t) => {
        ...
        if (cond) parts.push(`<text x="${labelX}" y="${rowY + 12}" fill="#8493A0" font-size="8">${esc(truncate(cond, maxChars + 4))}</text>`);
...
export function renderStateMachine(title: string, sm: StateMachineIR): FlowchartImage {
